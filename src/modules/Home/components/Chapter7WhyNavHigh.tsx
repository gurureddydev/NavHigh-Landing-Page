...
const StatCounter: React.FC<{ value: number; suffix: string; label: string; active: boolean }> = ({
    ...
    return (
        <div className="text-center">
            ...
            <div className="text-white/35 text-sm mt-2">{label}</div>
        </div>
    );
};
    ...
    return (
        <section ref={sectionRef} className="relative bg-[#040412] overflow-hidden py-32 md:py-40">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-14">
                ...
                >
                    {/* Others column */}
                    <div className="rounded-3xl border border-white/[0.04] bg-white/[0.015] p-8">
                        <div className="flex items-center gap-3 mb-8">
                            <div className="w-8 h-8 rounded-full border border-white/10 flex items-center justify-center">
                                <span className="text-white/20 text-lg leading-none">✕</span>
                            </div>
                            <span className="text-white/25 text-sm font-semibold tracking-wide uppercase">
                                Typical Agencies
                            </span>
                        </div>
                        <ul className="flex flex-col gap-3">
                            {OTHERS.map((item, i) => (
                                <li
                                    key={item}
                                    className="flex items-center gap-3 text-white/20 text-sm py-3 border-b border-white/[0.04] last:border-0 transition-all duration-500"
                                    style={{ transitionDelay: `${0.35 + i * 0.07}s`, opacity: inView ? 1 : 0 }}
                                >
                                    <span className="text-white/10 font-mono text-xs w-5 flex-shrink-0">
                    ...
                    >
                        ...
                        <div className="relative z-10">
                            <div className="flex items-center gap-3 mb-8">
                                ...
                                >
                                    <svg width="14" height="14" viewBox="0 0 256 256" fill="#3b82f6">
                            ...
                            <ul className="flex flex-col gap-3">
                                {NAVHIGH.map((item, i) => (
                                    <li
                                        key={item}
                                        className="flex items-center gap-3 text-sm py-3 border-b border-blue-500/[0.1] last:border-0 transition-all duration-500"
                                        style={{
                ...
                {/* Stats bar */}
                <div
                    className={`mt-16 grid grid-cols-1 sm:grid-cols-3 gap-8 py-12 border-t border-b border-white/[0.05] transition-[opacity,transform] duration-[1200ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
